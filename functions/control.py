...
def continuous_evolution_ab(r,sd,dif,gamma,T,L,M,N,theta,CI,show_graph_ini,show_graph_fin,mod_a,show_graph_x,b,ab,AbaB,AB):   
    ...
    for t in np.linspace(dt,T,M) : 
        ...
        if np.isin(True, ab_ > 0.5) and np.isin(True, ab_ < 0.99) :  
            position = np.append(position,np.where(ab_ > 0.5)[0][0])   # first position where the wave is over 0.5
...
def continuous_evolution_cd(r,sp,st,dif,gamma,T,L,M,N,theta,CI,show_graph_ini,show_graph_fin,mod_a,show_graph_x,cd,CdcD,CD): 
    ...
    for t in np.linspace(dt,T,M) : 
        ...
        if np.isin(True, cd_ > 0.5) and np.isin(True, cd_ < 0.99) :  
            position = np.append(position,np.where(cd_ > 0.5)[0][0])   # first position where the wave is over 0.5
